...
export type StyleInfo = {
  ...
  blinkPath?: string;
  /** 母音ごとの口パーツ（あいうえお）。無ければ mouthOpenPath を使う。 */
  mouthPaths?: Partial<Record<"n" | "a" | "i" | "u" | "e" | "o", string>>;
  engineId: EngineId;
...
export const rootMiscSettingSchema = z.object({
  ...
  // 立ち絵の口パク。simple=開閉のみ、vowel=母音パーツがある場合の詳細切り替え
  lipSyncMode: z.enum(["off", "simple", "vowel"]).default("simple"),
  lipSyncSpeed: z.enum(["slow", "normal", "fast", "max"]).default("normal"),
