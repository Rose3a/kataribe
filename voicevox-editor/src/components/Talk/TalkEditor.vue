    ...
    <QPageContainer>
      <QPage class="main-row-panes">
        ...
        <QSplitter
          ...
          unit="px"
          :limits="[
            shouldShowAudioDetail ? audioDetailPaneMinHeight : 0,
            shouldShowAudioDetail ? audioDetailPaneMaxHeight : 0,
          ]"
          separatorClass="home-splitter"
          :separatorStyle="{
            height: shouldShowPanes && shouldShowAudioDetail ? '3px' : '0',
          }"
          class="full-width"
          beforeClass="overflow-hidden"
          :disable="!shouldShowPanes || !shouldShowAudioDetail"
          :modelValue="shouldShowAudioDetail ? audioDetailPaneHeight : 0"
        ...
        >
          ...
          <template #after>
            <AudioDetail
              v-if="activeAudioKey != undefined && shouldShowAudioDetail"
              :activeAudioKey
            />
          </template>
        </QSplitter>
...
const DEFAULT_AUDIO_INFO_PANE_WIDTH = 300; // px
const MIN_AUDIO_INFO_PANE_WIDTH = 280;
const MAX_AUDIO_INFO_PANE_WIDTH = 420;
...
// Irodori-TTS is text-conditioned audio and has no VOICEVOX mora/pitch canvas.
// Keep the official editor layout while collapsing that pane.
const shouldShowAudioDetail = computed(() => false);
