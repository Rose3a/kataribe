        ...
        const getStyles = async function (
          ...
          for (const [i, style] of speaker.styles.entries()) {
            ...
            styles[i] = {
              ...
              mouthOpenPath:
                styleInfo.mouthOpen &&
                (await getResourceSrc(styleInfo.mouthOpen, "image")),
              blinkPath:
                styleInfo.blink &&
                (await getResourceSrc(styleInfo.blink, "image")),
              mouthPaths: styleInfo.mouthParts
                ? (Object.fromEntries(
                    await Promise.all(
                      Object.entries(styleInfo.mouthParts).map(
                        async ([shape, resource]) => [
                          shape,
                          await getResourceSrc(resource, "image"),
                        ],
                      ),
                    ),
                  ) as NonNullable<StyleInfo["mouthPaths"]>)
                : undefined,
    ...
    async action(
      ...
      if (
        ...
        newAudioItem.morphingInfo = baseAudioItem.morphingInfo
          ...
          : undefined;
        if (baseAudioItem.irodori != undefined) {
          newAudioItem.irodori = {
            ...baseAudioItem.irodori,
          };
        }
      }
  ...
  PLAY_CONTINUOUSLY_AUDIO: {
    action: createUILockAction(
      async ({ state, getters, mutations, actions }) => {
        ...
        const generationQueue = filterNonEmptyAudioKeys(
          state.audioKeys.slice(index),
          state.audioItems,
        );
        const player = new ContinuousPlayer(generationQueue, {
        ...
        player.addEventListener("generateerror", (e) => {
          void actions.RESET_PROGRESS();
          mutations.SET_AUDIO_NOW_GENERATING({
            audioKey: e.audioKey,
            nowGenerating: false,
          ...
          window.backend.logError(
            e.error,
            `Failed to generate audio for ${e.audioKey}.`,
          );
        });
  ...
  createPartialStore<AudioCommandStoreTypes>({
    ...
    COMMAND_CHANGE_AUDIO_TEXT: {
      ...
      async action(
        ...
        try {
          if (state.engineManifests[engineId]?.name.startsWith("Irodori")) {
            const freshQuery = await actions.FETCH_AUDIO_QUERY({
              text: skippedText,
              engineId,
              styleId,
            });
            mutations.COMMAND_CHANGE_AUDIO_TEXT({
              ...
              query: {
                ...query,
                ...freshQuery,
                speedScale: query?.speedScale ?? freshQuery.speedScale,
              },
            });
