...
ご提供いただいた音声データは、kataribe で利用する話者モデルの学習に使用します。学習した話者モデルは本リポジトリに標準話者として登録・配布し、各話者に定められた利用条件および本ツールの利用条件の範囲で、利用者が音声合成に利用できるよう公開します。この取り扱いに同意いただける場合にご応募ください。
...
4. **禁止事項・特記事項**
   ...
   - 既存のキャラクターガイドライン等がある場合は、その URL もお知らせください
...
`git clone` で導入した方は、アプリを終了した後、clone したディレクトリ内で `git pull` を実行することで更新できます。

GitHub Releases での配布も検討しています。
