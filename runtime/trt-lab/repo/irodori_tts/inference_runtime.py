...
from .meanflow import EncodedConditions, sample_euler_meanflow
...
class InferenceRuntime:
    ...
    def synthesize(
        ...
        *,
        ...
        reuse_encoded_conditions = not has_caption_text and not has_reference_input
        ...
        with (
            ...
            # Hold shared condition encodings. When manual_seconds bypasses duration
            # prediction, leave this unset and build the encoding in the RF stage.
            ...
            if self.model_cfg.flow_parameterization == "meanflow":
                z_patched = sample_euler_meanflow(
                    ...
                    # request conditions. Reuse them instead of encoding twice.
                    encoded_conditions=(
                        EncodedConditions(*encoded_conditions)
                        if encoded_conditions is not None else None
                    ),
                )
            else:
                z_patched = sample_euler_rf_cfg(
                    ...
                    # Reuse the condition encodings shared with duration prediction.
                    # Re-encode only when caption or reference input requires it.
                    encoded_conditions=encoded_conditions if reuse_encoded_conditions else None,
                    skip_caption_encoding=not has_caption_text,
                )
            stage_sec = _measure_end(self.model_device, t0)
            sample_stage = (
